...
import { buildBadgeComputeInput, loadUserBadgesWithSync, resolveFeaturedBadge } from '../lib/badges'
...
export function PublicProfilePage() {
    ...
    ;(async () => {
      ...
      try {
        ...
        try {
          ...
          const badgeInput = await buildBadgeComputeInput(
            fetched.id,
            fetched,
            cans,
            masters,
          ...
          const earned = await loadUserBadgesWithSync(fetched.id, badgeInput)
          if (active) setBadges(earned)
  ...
  const highlightedBadge = resolveFeaturedBadge(badges, profile?.featured_badge_id)
    ...
    <div className="min-h-dvh bg-monster-black">
      ...
      <main className="mx-auto max-w-lg px-4 py-4">
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  {highlightedBadge ? (
                    <BadgeCard badge={highlightedBadge} compact earned featured />
                  ) : null}
                  {badges.length > 0 ? (
          ...
          <Card className="p-4">
            <p className="mb-3 text-sm font-semibold text-white">Collector Badges</p>
            <BadgeShowcase
              badges={badges}
          ...
          {activeStats.featuredSet ? (
            <section>
              <p className="mb-2 text-sm font-semibold text-white">Featured Collection Set</p>
              <SetProgressCard progress={activeStats.featuredSet} linkToMissing={false} />
            </section>
          ) : null}

          {activeStats.setProgress.length > 0 ? (
            <section>
              <p className="mb-2 text-sm font-semibold text-white">Set Completion</p>
              <div className="flex flex-col gap-3">
                {activeStats.setProgress.slice(0, 5).map((row) => (
                  <SetProgressCard key={row.set} progress={row} linkToMissing={false} />
                ))}
              </div>
            </section>
          ) : null}
