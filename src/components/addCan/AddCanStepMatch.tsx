...
const offStatusCopy: Record<OffLookupStatus, { title: string; tone: 'success' | 'warning' | 'neutral' }> = {
  found: { title: 'Found on Open Food Facts (fallback)', tone: 'success' },
...
const primarySourceCopy: Record<AddCanStepMatchProps['primarySource'], string> = {
  ...
  open_food_facts: 'Open Food Facts (fallback)',
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-monster-muted">
        We check the verified master database first. Open Food Facts is only used when no master
        match exists.
      </p>
