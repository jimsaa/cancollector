...
/**
 * Match barcode against master_cans first. Query Open Food Facts when no master match exists.
 * Master lookup failures never block OFF — OFF does not require Supabase.
 */
export async function lookupBarcodeProduct(
  ...
  const master = await lookupMasterCan(trimmed)
  ...
  const fields = buildScanFormFields(trimmed, master, offProduct)
