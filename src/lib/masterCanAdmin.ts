...
/** Admin-only: update catalog reference image on a master can. */
export async function updateMasterCanReference(
  ...
  const reference_image_url =
    updates.reference_image_url !== undefined
      ? updates.reference_image_url?.trim() || null
      : master.reference_image_url?.trim() || null
  const reference_image_status = updates.reference_image_status ?? master.reference_image_status
  const off_preview_image_url =
    updates.off_preview_image_url !== undefined
      ? updates.off_preview_image_url?.trim() || null
      : master.off_preview_image_url?.trim() || null
